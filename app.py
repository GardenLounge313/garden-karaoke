...
🕒 Created: {booking.get('created_at', 'Now')}
    ...
    app.logger.info("About to connect to SMTP server...")
    try:
        server = smtplib.SMTP(EMAIL_HOST, EMAIL_PORT)
        app.logger.info("SMTP connection created, starting TLS...")
        server.starttls()
        app.logger.info("TLS started, logging in...")
        server.login(EMAIL_USER, EMAIL_PASSWORD)
        app.logger.info("Login successful, sending email...")
        server.sendmail(EMAIL_USER, NOTIFICATION_EMAIL, msg.as_string())
        server.quit()
        app.logger.info(f"✅ Email notification sent for booking #{booking['id']}")
        return True
